...
| 항목 | 판정 |
|------|------|
| 13개 기준 테이블 대응 | **조건부 승인** — `point_packages` 미구현, 나머지는 매핑·확장으로 대응 |
...
| point_packages | *(없음)* | 누락 | **Stage 1 보정** | `calculate_revenue_split()` 함수로 배분 로직만 구현. 선납 건별 패키지 기록 테이블은 Stage 1 |
...
| `point_packages.*` | `calculate_revenue_split()` 반환값 | Stage 1 테이블화 예정 |
...
13 (기준)
 ...
 -1  point_packages      → 함수만 구현 (테이블 누락)
...
| 쟁점 | 우선순위 | 제안 |
|---|---|---|
| `point_packages` 테이블 미구현 | **높음** | Stage 1 migration으로 선납 건별 배분 스냅샷 추가 |
| `user_regions` 단일 지역 | 중 | MVP OK. 다중 지역 요구 시 junction 복원 |
| `ledger_entry_type` 명칭 | 낮 | API DTO에서 charge/quiz_reward alias |
| DB live 검증 미실행 | **높음** | Docker 설치 후 `supabase db reset` + validate 재실행 |
| 광고주 퀴즈 REST 노출 | 중 | Stage 1 BFF에서 consumer는 `quizzes_public` 강제 |
