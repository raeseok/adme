/**
 * Stage 2-B — ad_views mutation must not exist in Stage 2-B code paths
 */
...
const MUTATION_PATTERNS = [
  /\.from\(\s*["']ad_views["']\s*\)\s*\.(insert|update|delete|upsert)/,
  /ad_views.*\.(insert|update|delete|upsert)/i,
];
...
if (failed === 0) {
  console.log("PASS: Stage 2-B paths — no ad_views mutation");
}

console.log(
  "INFO: runtime DB ad_views count not available due to RLS/env — static scan + diagnostics marker verified",
);

if (failed > 0) {
  process.exit(1);
}
console.log("PASS: verify:stage2b-no-ad-views-mutation");
