/**
 * Stage 3-1 — stage30 markers only on /admin/diagnostics (Production)
 */
...
const PUBLIC_FORBIDDEN = [
  ...
  "stage30ExpectedDevSupabaseRef",
];

const DIAGNOSTICS_REQUIRED = [
  "stage30Build=stage3-0-supabase-ledger-safety-readiness-production",
  "stage30DevProdSupabaseSeparated=true",
  "stage30ReadinessStatus=READY_FOR_STAGE3_DESIGN_ONLY",
...
async function main() {
  ...
  try {
    ...
    console.log("PASS: verify:stage3-1-public-marker-guard");
  } finally {
    await browser.close();
  }
}
