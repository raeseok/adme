/**
 * Stage 3-D — Production reward actual mutation must remain blocked
 */
...
import { STAGE3D_DIAGNOSTICS_REQUIRED } from "./utils/stage3d-helpers.mjs";
...
async function main() {
  ...
  assertMarkerList(
    ...
    "Production reward-preflight Stage 3-D",
  );
  ...
  // summary section may omit current ref; fall back to stage3C marker
  const ref =
    currentRef ||
    extractMarkerValue(sources.combined, "stage3CCurrentSupabaseProjectRef");
  if (ref && ref !== KNOWN_PROD_SUPABASE_REF) {
    throw new Error(
      `Production must use prod ref ${KNOWN_PROD_SUPABASE_REF}, got ${ref}`,
    );
  }
  console.log("PASS: Production diagnostics Stage 3-D markers");
  ...
  const email = `stage3d-prod-${Date.now()}-${randomBytes(3).toString("hex")}@example.com`;
  ...
  if (!rpc.error.message.includes("STAGE3B_PRODUCTION_BLOCKED")) {
    throw new Error(
      `expected STAGE3B_PRODUCTION_BLOCKED, got ${rpc.error.message}`,
    );
  }
  console.log("PASS: Stage 3-B Production RPC blocked");
  ...
  const settlementsAfter = await countTable(supabase, "partner_settlements");
  const cashAfter = await countTable(supabase, "cash_redemption_requests");
  ...
  if (settlementsAfter.count !== settlementsBefore.count) {
    throw new Error("Production partner_settlements mutated");
  }
  if (cashAfter.count !== cashBefore.count) {
    throw new Error("Production cash_redemption_requests mutated");
  }

  await verifyProductionUiBlocked();

  console.log("RESULT: stage3CProductionRewardBlocked=true");
  console.log("RESULT: stage3BProductionBlocked=true");
  ...
  console.log("PASS: verify:stage3d-production-reward-blocked");
}
