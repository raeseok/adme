/**
 * Stage 2-C — no Kakao actual send integration, SDK, env, or phone collection
 */
...
const CODE_SCAN_ROOTS = [
  join(webRoot, "src/lib/consumer-ads/stage2c-ad-views.server.ts"),
  join(webRoot, "src/lib/consumer-ads/stage2c-fixture-views.server.ts"),
...
for (const dir of CODE_SCAN_ROOTS) {
  ...
  const files = st.isDirectory() ? walk(dir) : [dir];
  for (const file of files) {
    if (file.includes("verify-stage2c-kakao-no-send")) continue;
...
const stage2cDoc = join(repoRoot, "docs/adme/stage-2-c-ad-views-server-min-view.md");
if (!statSync(stage2cDoc, { throwIfNoEntry: false })) {
  console.error("FAIL: missing stage-2-c doc");
  failed++;
} else {
  console.log("PASS: stage-2-c doc exists");
}

if (failed === 0) {
  console.log("PASS: no Kakao API/SDK/env/phone UI in Stage 2-C code");
  console.log("PASS: verify:stage2c-kakao-no-send");
} else {
  process.exit(1);
}
