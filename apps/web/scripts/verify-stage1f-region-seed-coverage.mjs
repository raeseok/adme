...
async function main() {
  const manifest = JSON.parse(
    await readFile(path.join(__dirname, "regions/source/source-manifest.json"), "utf8"),
  );
  ...
  const markers = [
    ...
    `stage1FSourceKind=${manifest.sourceKind ?? "molit-legal-dong"}`,
    `stage1FSourceEffectiveDate=${manifest.effectiveDate}`,
